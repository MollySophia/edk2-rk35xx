    ...
    )
{
  ...
  if (RdInterval > 4)
    DEBUG ((DEBUG_ERROR, "AUX interval %d, out of range (max 4)\n", RdInterval));
  ...
  )
{
  switch(LinkBw) {
  default:
    DEBUG ((DEBUG_ERROR, "Unknown DP link BW code %x, using 162000\n", LinkBw));
  ...
  )
{
  ...
  if (Reg & PLL_LOCK) {
    ...
    DEBUG ((DEBUG_ERROR, "Pll unlocked \n"));
    return PLL_UNLOCKED;
	}
}
  ...
  )
{
  ...
  if ((Dp->LinkTrain.LinkRate != DP_LINK_BW_1_62) &&
    ...
    DEBUG ((DEBUG_ERROR, "faiied to get Rx Max Link Rate  0x%2x \n",
          Dp->LinkTrain.LinkRate));
    return -ENODEV;
  }

  if (Dp->LinkTrain.LaneCount == 0) {
    DEBUG ((DEBUG_ERROR, "faiied to get Rx Max Lane count\n"));
    return -ENODEV;
  }
  ...
  )
{
  ...
  if (AnalogixDpClockRecoveryOk(LinkStatus, LaneCount) == 0) {
    ...
    DEBUG ((DEBUG_ERROR, "Link Training Clock Recovery success\n"));
  ...
  )
{
  ...
  if (AnalogixDpClockRecoveryOk(LinkStatus, LaneCount)) {
    DEBUG ((DEBUG_ERROR, "AnalogixDpReduceLinkRate \n"));
    AnalogixDpReduceLinkRate(Dp);
    //Todo
    //return -EIO;
  }
  ...
  if (!AnalogixDpChannelEqOk(LinkStatus, LinkAlign, LaneCount)) {
    ...
    DEBUG ((DEBUG_ERROR, "Link Training success\n"));
    ...
    DEBUG ((DEBUG_ERROR, "Final link rate = 0x%.2x, Lane count = 0x%.2x\n",
  ...
  )
{
  ...
  while (!Retval && !TrainingFinished) {
    ...
    case START:
      ...
      if (Retval)
        DEBUG ((DEBUG_ERROR, "LT link start failed!\n"));
      break;
    case CLOCK_RECOVERY:
      Retval = AnalogixDpProcessClockRecovery(Dp);
      if (Retval)
        DEBUG ((DEBUG_ERROR, "LT CR failed!\n"));
    case EQUALIZER_TRAINING:
      Retval = AnalogixDpProcessEqualizerTraining(Dp);
      if (Retval)
        DEBUG ((DEBUG_ERROR, "LT EQ failed!\n"));
    case FINISHED:
      TrainingFinished = 1;
      break;
    case FAILED:
  ...
  )
{
  ...
  for (i = 0; i < 5; i++) {
    ...
    if (Ret < 0) {
      DEBUG ((DEBUG_ERROR, "faiied to init training\n")); 
      return Ret;
    }
  ...
  )
{
  ...
  Reg = AnalogixDpGetPllLockStatus(Dp);
  if (Reg != PLL_LOCKED ) {
    DEBUG ((DEBUG_ERROR, "PLL is not locked yet. \n"));
    Ret = AnalogixDpRegRead(Dp, ANALOGIX_DP_DEBUG_CTL);
    DEBUG ((DEBUG_ERROR, "PLL Dp debug ctl ret 0x%.8x\n", Ret));
  }
  ...
  )
{
  ...
  if (Ret) {
    DEBUG ((DEBUG_ERROR, "Falied to read dpcd cap \n"));
    return Ret;
  }

  Ret = AnalogixDpLinkPowerUp(Dp);
  if (Ret) {
    DEBUG ((DEBUG_ERROR, "Falied to link power up \n"));
    return Ret;
  }
  ...
  if (Ret) {
    DEBUG ((DEBUG_ERROR, "Unable to do link train !!! \n"));
    return Ret;
  }
